...
S cs="()+-<>~,", prelude="(E)(D)(C)(B)(A)"; I pl=(sizeof prelude)-1;
#define prog(i) I _j=i; DO(l,s[i+pl]=cs[_j%8];_j/=8)
I oflen(I l) { char *s=MA(l+pl+1); strcpy(s,prelude); s[l+pl]=0; T ast; D r=newRoot();
	DO(ipow(8,l), prog(i); parses(s,&ast)&&small(&ast,r)&&comb(ast)&&PF("%s|",s)&&pr(ast); freeTerms(ast)); freeRules(r); FR(s); }
I main(I argc,S argv[]){ P(argc==1,(puts("usage: ./bible <length>"),0)); DO(1+atoi(argv[1]),oflen(i)); }
