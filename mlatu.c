...
I len(T t) { if (t->t==TRM) R strlen(t->w); else { I i=2; MAP(t->c,i+=len(c)+!!c->n) R i; } } // length of printed T
I prT(T t,C s); I prTL(T t,C s) { I i=0; MAP(t,i+=prT(c,s+i);if(c->n)s[i++]=' '); R i; } // print T list
I prT(T t,C s) { if (t->t==TRM) R strlen(strcpy(s,t->w)); else { *s='('; I i=prTL(t->c,++s); s[i]=')'; R i+2; } } // print T
...
V rm(T t) { T n=t->n->n; fT(t->n); t->n=n; } // removes next T
V zap (T t) { rm(t); rm(t); } 
...
I lit(T t,D r) { D cR=r, bst=0; T u=t->n; // try to find rewrite on terms starting with literal
...
I qot(T s) { T t=s->n, u; if (!(u=t->n)) R 0; // try to find rewrite on terms starting with quote
