...
I lst; V prML(M m); V prM(M m) { if (lst==Q) lst=TRM; else PF(" "); switch (m->t) { 
	case TRM: PF("%s",m->w); break; case Q: PF("("); lst=Q; prML(m->c); PF(")"); break; } }
...
I main(I ac,C *av) { D a=nID(wP,0,"?a\0"), r=nD("r\0",rPtv), d=nD("d\0",dPtv), q=nD("q\0",qPtv),
		a2=nID(wP,0,"?a\0"), s=nD("s\0",sPtv), 
		q1=nID(qP,0,"?q\0"), u=nD("u\0",uPtv), q2=nID(qP,0,"q\0"), c=nD("c\0",cPtv);
	nC(a,r); nC(a,d); nC(a,q); nC(a,a2); nC(a2,s); a->n=q1; nC(q1,u); nC(q1,q2); nC(q2,c);
	for (I i=0;i<ac;i++) { if (!strcmp("-d",av[i])) dbg=1; }
	C t=ma(100); I i, l;
	while (gets(t)) { l=ch=i=0; M ast=nM(ST,""); P(t,&i,&ast->n,0); ex(ast,a); MAP(ast,l++);
		if (l==2&&!strcmp("bye",ast->n->w)) exit(0); if (!dbg||!ch) prAST(ast); fML(ast); }
	fD(a); }
