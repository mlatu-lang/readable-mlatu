...
#define PRS(nm,fn,prel,cur,slce,next,end) E nm##H(S s,D root,T rs,T y) { S f=fn, n; prel;                                  \
	 I l=0, r=0, w=0, x=0, cm=0, ln=1, c, m; do { c=cur; l++; cm=cm?c!='\n':c=='#'; if (!WS(c)&&!cm) r=1;                       \
		...
		if (w==7) { x++; if (WS(c)) {                                                                                           \
			S nS=MA(m=x); slce; nS[x-1]=0; MAP(y,); c->n=nT(0,nS); E e=parseFileH(nS,root,rs,y); FR(nS); P(e.e,end,e); w=x=0; } } \
		else if ("#wield "[w]==c) w++; else w=0; next; } while (c>0);                                                           \
	end; P(r,ER(PRD,ln)); R (E){}; }                                                                                         \
E nm(S s,D root,V(*cb)(S)) { T rs=nT(0,""), y=nT(0,""); E e=nm##H(s,root,rs,y);                                            \
...
PRS(parseFile,  s,  FILE*a=fopen(s,"rb");P(!a,ER(OPEN,0)), fgetc(a), fseek(a,-m+(c<0),SEEK_CUR);fread(nS,1,m,a),    , fclose(a));
